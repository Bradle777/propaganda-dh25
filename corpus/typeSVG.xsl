<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
        ...
        select="collection('./Valid Markup? select=*.xml')"/> <xsl:template name="xsl:initial-template">
            <root>
                ...
                <svg
                    ...
                    <text x="{$bruhh}" y="-550" fill="black">Number of times foreign policy was brought up</text>
                    ...
                    <xsl:for-each select="0 to 5">
                        ...
                        <xsl:variable name="val" as="xs:double" select="(position()-1)*10"/>
    ...
    <xsl:template match="/">
        ...
        <xsl:variable name="ypos" as="xs:double" select="(count(//policy[@type='f'])*10)"/>
        <!-- LEFT TO DO:  DIP, 
            TYPE: F D 
        Connotation maybe-->
